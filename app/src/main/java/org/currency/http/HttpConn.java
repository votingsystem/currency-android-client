...
public class HttpConn {
    ...
    public ResponseDto doPostForm(RequestBody formBody, String url) {
        //RequestBody formBody1 = new FormBody.Builder().add("key1", "val1").add("key2", "val2").build();
        ResponseDto responseDto = null;
        try {
            LOGD(TAG + ".doPostForm", "url: " + url);
            Request request = new Request.Builder().url(url).post(formBody).build();
            Response response = client.newCall(request).execute();
            ContentType respContentType = ContentType.getByName(response.header("Content-Type"));
            LOGD(TAG + ".doPostForm", "status: " + response.code() + " - respContentType: " + respContentType);
            responseDto = new ResponseDto(response.code(), response.body().bytes(), respContentType);
        } catch (IOException ex) {
            ex.printStackTrace();
            responseDto = new ResponseDto(ResponseDto.SC_ERROR, ex.getMessage());
