...
public class DNIeContentSigner implements ContentSigner {
    ...
    @Override public AlgorithmIdentifier getAlgorithmIdentifier() {
        return new DefaultSignatureAlgorithmIdentifierFinder().find(Constants.SIGNATURE_ALGORITHM);
    }
    ...
    private class SignatureOutputStream extends OutputStream {
        ...
        byte[] getSignature() {
            byte[] sigBytes = null;
            try {
                Signature signature = Signature.getInstance(Constants.SIGNATURE_ALGORITHM, "DNIeJCAProvider");
    ...
    public static CMSSignedData signData(PrivateKey privateKey, X509Certificate userCert,
        ...
        dnieSignerInfoGeneratorBuilder = dnieSignerInfoGeneratorBuilder.setProvider(Constants.PROVIDER);
