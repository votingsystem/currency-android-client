...
public class CurrencyContentProvider extends ContentProvider {
    ...
    private static final String DB_NAME       = "currency_content_provider.db";
    private static final String TABLE_NAME    = "currency";
    public static final String AUTHORITY      = "currency.org.currency";
    ...
    public static void updateDatabase(Currency currency) throws Exception {
        Uri uri = App.getInstance().getContentResolver().insert(CurrencyContentProvider.CONTENT_URI,
                CurrencyContentProvider.getContentValues(currency));
        LOGD(TAG + ".updateDatabase", "uri: " + uri + " - state: " + currency.getState());
    }
