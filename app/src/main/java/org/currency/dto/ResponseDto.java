...
public class ResponseDto<T> implements Parcelable {
    ...
    public ResponseDto(Parcel source) {
        ...
        byte[] cmsMessageBytes = new byte[source.readInt()];
        source.readByteArray(cmsMessageBytes);
        if(cmsMessageBytes.length > 0) {
            try {
                cmsMessage = CMSSignedMessage.FROM_PEM(cmsMessageBytes);
            } catch (Exception ex) {
                ex.printStackTrace();
            }
        }
    ...
    public ResponseDto(int statusCode, CMSSignedMessage cmsSignedMessage) {
        this.statusCode = statusCode;
        this.cmsMessage = cmsSignedMessage;
    }
    ...
    public String getCaption() {
        if (caption == null && operation != null) return operation.getOperation().getType().name();
        else return caption;
    }
